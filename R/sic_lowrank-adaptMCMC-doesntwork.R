...
sic_lowrank = function(formula,
  ...
  incidence_draws = 
    MCMC(p = logpost,
         ...
         init = init_vector,
         acc.rate = 0.234)
  colnames(incidence_draws$samples) = 
    c(paste(rep(pathogen_vars,each = Q),rep(c("Intercept",X_vars),P),sep=":"),
      paste("U",rep(pathogen_vars,lrank),rep(1:lrank,each = P),sep=":"),
      paste("V",rep(pathogen_vars,lrank),rep(1:lrank,each = P),sep=":")
    )
  user_is_happy = !get_user_input
  while(!user_is_happy){
    
    cat("\nLook at select trace plots, and select burn-in value.")
    mcmc_trace(incidence_draws$samples[,1:(P*Q)])
    
    burnin = NA
    while(is.na(burnin)){ burnin = as.numeric(readline()); cat("\nEnter a number.")}
    cat("\nHit ENTER to look at ")
    # How many additional samples do you want?  If none, enter 0.\n")
    add_samples = as.numeric(readline())
    if(is.na(add_samples)){
      incidence_draws = 
        MCMC.add.samples(incidence_draws,add_samples)
      
    }else{
      cat("Type a number and hit ENTER\n")
    }
  }
